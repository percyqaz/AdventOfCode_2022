...
fn main() {
	...
	let mut intermediate_moving_stack : LinkedList<String> = LinkedList::new();
		...
		for line in lines {
			...
			// THIS SECTION MOVES BOXES AROUND BASED ON THE INSTRUCTION
				...
				for _ in 0..amount {
					...
					intermediate_moving_stack.push_front(moved_item);
				}

				for _ in 0..amount {
					let moved_item = intermediate_moving_stack.pop_front().unwrap();
