...
module Reader =
    ...
    let parse_monkey() : Monkey =
        ...
        { Items = startingItems; Operation = operation; Test = test; IfTrue = if_true; IfFalse = if_false; Inspections = 0 }
...
module Solution =
    ...
    let main() =
        for i = 1 to 20 do round(i)
        monkeys
        |> Array.map (fun monkey -> monkey.Inspections)
        |> Array.sortDescending
        |> fun x -> printfn "%A" x; x
        |> fun xs -> xs.[1] * xs.[0]
        |> printfn "Monkey business: %i"
