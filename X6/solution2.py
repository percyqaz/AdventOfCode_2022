...
def single_actor_score(part_of_graph):
    global best_score
    best_score = 0
    ...
    paths(current, 26, 0, part_of_graph)
...
count = 2 ** (len(smaller_graph) - 1)
best = 0

for s in range(count):
    print(s/count*100, "%")
    left, right = graphs(smaller_graph, s)
    score = single_actor_score(left) + single_actor_score(right)
